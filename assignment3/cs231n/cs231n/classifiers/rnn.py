...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        # (2)
        wemb, cache_wemb = word_embedding_forward(captions_in, W_embed)
        # (3)
        if self.cell_type == 'rnn':
            h, cache = rnn_forward(wemb, h0, Wx, Wh, b)
            
        if self.cell_type == 'lstm':
            h, cache = lstm_forward(wemb, h0, Wx, Wh, b)
        ...
        # Backward pass
        dx_voc, dw_voc, db_voc = temporal_affine_backward(dout, cache_out)
        if self.cell_type == 'rnn':
            dx, dh0, dWx, dWh, db = rnn_backward(dx_voc, cache)
        if self.cell_type == 'lstm':
            dx, dh0, dWx, dWh, db = lstm_backward(dx_voc, cache)
            
        dW_wemb = word_embedding_backward(dx, cache_wemb)
        _, dw_proj, db_proj = affine_backward(dh0, cache_h0)
        
        grads['W_vocab']=dw_voc
        grads['b_vocab']=db_voc
        grads['Wx']=dWx
        grads['Wh']=dWh
        grads['b']=db
        grads['W_embed']=dW_wemb
